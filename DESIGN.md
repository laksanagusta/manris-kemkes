...
- **Warning Card:** Use the shared `WarningCard` for persistent attention messages. It keeps the standard `rounded-lg` and `shadow-custom` boundary while using `warning-card-surface` (`#FBF6E8`) with `warning-card-foreground` (`#000000`) for readable black text.
...
- **Brand / Wordmark** (`font-logo text-[20px] leading-5 font-semibold lowercase tracking-[-0.4px]`): The global topbar uses the Poppins 20px semibold lowercase wordmark (`manris`) without a logo image, aligned to the sidebar menu inset. Public authentication screens, including registration, reuse the same text-only lowercase wordmark. The canonical 4×4 mark remains reserved for the mobile sidebar, while the browser-tab icon uses a standalone nearly-black rounded square with a subtle gray edge gradient for depth.
...
- **Dashboard KPI card title** (`font-sans text-[11px] font-semibold uppercase tracking-[1px] leading-4 text-muted-foreground`): Dashboard KPI card titles use Inter at 11px with `muted-foreground`, semibold weight, uppercase casing, and 1px letter spacing. The card uses 20px padding and a 24px separation before the value row. Numeric values use Inter at 28px with semibold weight and `foreground`, with no inline comparison marker. Collection `KpiCard` labels retain their separate collection-card treatment.
...
- `rounded-lg` (8px): Canonical radius for inputs, selects, textareas, search, toolbar buttons, cards, panels, popovers, dialogs, form sections, premium controls, and large action zones.
- `rounded-xl` (10px): Optional intermediate radius for an explicitly documented exception; it is not the default surface radius.
- `rounded-2xl` (16px): Reserved for exceptional surfaces only; standard components use `rounded-lg`.
...
- **Labeled buttons:** All labeled buttons are text-only with no decorative leading or trailing icon. Loading states communicate through the loading label and `aria-busy`; icon-only controls use `ActionIconButton` or an explicit `icon-*` button size.
...
### Cards
- **Default:** `rounded-lg` (8px) with 16px padding, white surface, and the shared `shadow-custom` boundary.
- **Large:** `rounded-lg` (8px) with 24px padding.
...
- **Dashboard Card:** `rounded-lg` with `bg-card` and the shared `shadow-custom` boundary. Dashboard KPI cards use a `min-h-[100px]` baseline without an embedded chart, `px-5 py-5` padding, and a compact 11px semibold uppercase dashboard-label treatment with 1px letter spacing. The title uses Inter semibold uppercase text, and the value uses 28px Inter semibold tabular numerals. The card may grow when a long title wraps. Collection `KpiCard` keeps its separate collection label treatment. Chart and panel cards keep the roomier `px-4 py-4` header band with no divider line. Risk-analysis summary, chart, and history surfaces use the same shared shadow boundary.
- **Card Header:** Dashboard KPI cards own 20px inset padding, use `font-sans text-[11px] font-semibold uppercase tracking-[1px] leading-4 text-muted-foreground` for the title, and start the value row with `mt-6 flex items-baseline gap-1` for a 24px title-to-value gap. The KPI value uses `text-[28px] font-semibold leading-none tracking-tight text-foreground` with no inline comparison marker. Collection `KpiCard` retains its own label/value spacing and does not render subtitle or description content. Header-and-content cards must use `gap-0 p-0` on the card shell and one `p-4` content inset so the component's default gap does not stack with content padding.
...
- **Collapsible Card:** Use the shared `CollapsibleCard` compound component for operational disclosures. Compose `Root`, `Trigger`, `Header`, `Icon`, `Text`, `Title`, optional `Description`, optional `Actions`, `Content`, and `Body` through children instead of adding boolean display modes. The shell uses `rounded-lg`, a full-width 16px-inset trigger, a 32px circular chevron with the shared outline-button perimeter (`border border-border bg-card text-foreground` and the standard subtle button shadow), a 2px chevron stroke, 14px semibold title, optional 12px description, a light body divider, and a 200ms grow/collapse animation with reduced-motion fallback. Put period and other inline context metadata in `CollapsibleCard.Actions` using the shared compact neutral Context Badge treatment. Interactive header controls such as `PopoverSelectField` must remain siblings of `Trigger` in a shared header row rather than nesting button-based controls inside the collapsible trigger.
...
### Overlays and Groups
- **Modal:** `rounded-lg` with 20px padding and the shared smooth elevation.
- **Modal family:** All `Dialog`, `AlertDialog`, and `Sheet` surfaces use the solid `bg-card` surface, 20px (`p-5`) outer inset, `rounded-lg` geometry where the surface is centered, the shared `smooth-shadow-ring-xl shadow-black smooth-ring-neutral-300/30` elevation, the frosted scrim, hidden scrollbar chrome, and the shared 200ms strong ease-out transition. Preserve explicit width and scrolling variants only when the content requires them; do not introduce a second local modal shell.
...
- **Risk cascade action dialog:** The create and review flows reuse the canonical Dialog shell at `max-w-2xl` with `no-scrollbar`, no close icon, a compact title/description header, 20px outer inset, and an internal footer divider. Create fields use the shared 40px field geometry with `flex flex-col gap-2` label-to-control rhythm. Review context is one quiet `bg-muted/20` metadata surface with a single inset ring, compact neutral cascade-type context badge, and semantic status badge; source and target values wrap within their columns. The footer uses `CollectionDialogCancel` for `Batal`, `AccentButton` for `Simpan eskalasi`/`Setujui`, and `DestructiveButton` for `Tolak`; do not use route-local badge color classes or a duplicate close control.
...
- **Shared modal shell:** `DialogContent` uses `w-[calc(100%-2rem)] max-w-2xl max-h-[calc(100dvh-2rem)] overflow-y-auto overscroll-contain no-scrollbar rounded-lg bg-card p-5` with `smooth-shadow-ring-xl shadow-black smooth-ring-neutral-300/30`; `AlertDialogContent` keeps the same treatment at `max-w-lg`. The compound header uses a compact `gap-1` rhythm with `px-5 pt-5 pb-3` inset so its title has bottom breathing room, while the compound footer uses an internal `border-t border-border/70` divider, `px-5 py-4` inset, and compact actions. Explicit `max-w-*` overrides remain available for intentionally expanded dialogs.
...
- **Popover:** `rounded-lg` with solid popover surface and subtle ring. All `PopoverContent` instances inherit this radius; feature overrides must not replace it with another radius.
...
- **Sidebar icon motion:** `SidebarNavItem` is the reusable motion primitive for Dashboard, Library, Search, and all operational navigation. On fine-pointer hover, the icon uses a 180ms `cubic-bezier(0.2, 0, 0, 1)` `scale(1.08)` plus `translateY(-1.5px)`; press feedback reaches `scale(0.92)` and settles with a 350ms spring (`bounce: 0.18`). The SVG stroke width changes subtly through the same 180ms transition, and inactive icons transition from `muted-foreground` to the active foreground color on hover/press. Inactive icons and labels use `muted-foreground` (`#8F8E8E`), with `font-medium` labels and `1.8` icon stroke weight; active icons follow the foreground active label color at the same medium weight. The neutral active surface uses `layoutId` with the same spring so it travels between items without changing layout; do not add a decorative left indicator. Sidebar navigation starts with an 8px top inset (`pt-2`) below the header/logo. Sidebar menu rows use a compact 4px vertical gap so hover surfaces remain visually distinct, while top-level menu groups use a 12px vertical gap (`gap-3`) to make the hierarchy legible.
...
- **Intelligence collection pages** such as MoM and Document Intelligence use the same `PageStack` + `CollectionToolbar` shell as the risk register. Keep their workflow cards on the neutral `rounded-lg` collection surface, with actions and review states inside that shared rhythm.
...
- **Document Intelligence findings list:** Keep findings in a flat list without priority grouping, aggregate priority summary, or an additional outer card wrapper; use the shared `bg-card` surface for each finding card, render severity with the borderless Design System `Badge` primitive using semantic tones, retain only `Keyakinan` in the compact metadata row, keep the finding summary flush-left under its title, and render `Buat draf risiko` as an outlined secondary action inside a modal-like card footer with a top divider. The result header keeps its outline `Mulai proses baru` action on the canonical `border-0 border-shadow` / `shadow-custom` treatment without an additional hard border.
...
- **Document Intelligence workspace:** Treat the document as the primary object, not a single upload form. Use one centered upload-first layout for setup, active processing, and completed findings; the duplicate global `AppHeader` and local `CollectionPageHeader` remain suppressed, while the setup heading keeps the immediate context at 16px (`text-base`) with an 8px (`space-y-2`) gap to its subtitle, and the drop-zone heading remains compact at 14px (`text-sm`). Keep the workspace content close to the top of the shared `PageStack` rhythm rather than reserving a standalone header band. Keep only `Mode analisis` visible directly above the upload surface with a 24px gap, as a responsive two-column grid of two radio cards from `sm` upward (one column on narrow screens) with the options `SOP` and `Laporan Mitigasi`; the legend and option titles use the same compact 14px scale. Mode cards are icon-free, place the radio control in the top-right corner, and keep the selected card on the same `bg-card` background as the unselected card; active state is communicated by the border only, while unselected cards retain visible hover and focus states. The processing period is generated automatically rather than exposed as a field. Give one large neutral `rounded-lg` dashed drop zone visual priority; use a restrained document glyph as the only non-semantic accent, concise Indonesian instructions, a 14px drop-zone heading, keyboard activation, visible focus, and reduced-motion-safe drag feedback. The drop zone exposes one keyboard target only: its programmatically activated file input stays outside the button semantics and outside the tab order. After a valid selection, replace the drop zone with one compact file row containing type thumbnail, filename, extension, size, removal control, validation feedback, and a card-matched `border-shadow`; do not simulate network upload progress. File rows enter and leave with an interruptible 180ms opacity/full-transform tween using `--ease-out`; reduced motion retains a 120ms opacity-only transition. Keep the single layout free of a setup or history accordion, preserve the valid selected file when an invalid additional file is attempted, retain the one-document and 1 MB limits, and make `Mulai analisis` the sole primary setup action. The quarterly cycle still generates the process name automatically without a manual name or organization-ID field. Submit the selected PDF or XLSX directly to the backend Document Intelligence API with the authenticated session and organization context; do not use deterministic mock processing or a client-side synchronization fallback. During processing, show only a compact status card with the current stage, completed-task count, cancel action, and native `progressbar` semantics with concise polite announcements; render progress through a left-origin `scaleX()` transform over 200ms instead of animating width. Do not render the spatial index, parallel task lanes, activity timeline, history, or inspector. When analysis reaches a terminal state, keep the same upload-first layout, clear the processed file from the drop zone, and show only the `Temuan untuk ditinjau` panel below it with the workspace's 32px (`space-y-8`) direct-section rhythm. Use the result-transition rule above for its entrance and reduced-motion fallback. Let each finding disclose its source quote and recommended action inline, and keep `Buat draf risiko` as the only finding action. Keep all ministry-facing workflow labels in Indonesian, body/supporting text at 14px, and operational metadata/status at no less than 12px. Persist only terminal job summaries locally for the visual result state; an active API request is not resumable from storage because the selected `File` is browser-owned. Surface backend failures in the terminal result panel and keep cancellation mapped to the request's abort signal.
...
- Application data tables use the shared ledger geometry: header cells use `text-[13px]` labels (13px), `font-medium` (500), capitalize casing, `0.05em` tracking, `py-1.5 px-6`, the `table-header` token (`#FCFCFC`) as the header band, and `text-secondary-foreground`; data cells use `py-4 px-6`. Every data-row list uses an internal `border-b border-border/60` divider applied to its cells by `TableBody`, with the trailing divider omitted. The dashboard `Risiko yang Perlu Perhatian` ledger uses the same header and data-cell geometry (`px-6 py-1.5` / `px-6 py-4`) with the 13px label scale (`text-[13px]`), while retaining the neutral header surface and `text-secondary-foreground`. The table primitive normalizes every other header and nested header control to this 13px/500 scale, while all body cell content remains `font-normal`; table body text defaults to `text-muted-foreground`, and row titles may opt back into `text-foreground` explicitly. Badge components keep their own semantic `tone` colors and are not overridden by the table text rule. Header bottoms and footer tops use `border-border/60`. `CollectionTableHead density="compact"` changes only the row geometry: its header text and `py-1.5 px-6` padding remain canonical. Sortable header controls inherit the same `capitalize` treatment, use no additional horizontal inset so their labels align with body cells, and must not add `uppercase`. For compact collection headers, `CollectionTableHeader density="compact"` owns a 40px row without changing the shared header padding.
...
- Register table cards begin directly with the table header, without an empty padded title band. Their outer shell uses `rounded-lg bg-card` with the shared `border-shadow` treatment through `CollectionTableCard`. Keep the header divider at `border-b border-border/60` and avoid layered outer borders or rings.
...
- Dashboard KPI cards do not contain charts or inline trend/comparison indicators. Keep overview KPI labels concise and specific (`Total`, `Prioritas`, `Mitigasi belum terlapor`, and `Eksposur`) so the metric grid remains scannable. `Mitigasi belum terlapor` binds to the dashboard's `unreportedMitigations` count, which includes mitigation tasks without a valid report regardless of whether their due date has passed; it must not reuse the overdue-only count. Use an 11px semibold uppercase dashboard label with 1px letter spacing, a 28px KPI value, and `—` when the value is unavailable. Collection KPI labels retain their separate collection-card treatment. Use the dedicated dashboard charts below the KPI grid for trend analysis.
...
- **Risk Event Ledger (LED):** Pencatatan kejadian memakai bottom `Drawer` berbasis Vaul sebagai tray bertahap, bukan modal panjang satu layar. Gunakan shell inset `8px` dengan tinggi dinamis yang mengikuti ukuran form aktif dan batas aman viewport (`max-h-[calc(100dvh-1rem)]`) serta `max-w-3xl` yang sama dengan lebar form, sehingga tray mengikuti komponen di dalamnya; pada mobile lebar menyusut mengikuti viewport. Perubahan tinggi memakai `layout animation`/`continuity transition` yang interruptible: drawer grow saat langkah atau disclosure membuka konten, lalu shrink saat konten dipadatkan, dengan `--ease-out` 220ms dan fallback reduced-motion tanpa transisi. Sertakan handle drag canonical dan footer sticky; header memakai `DrawerTitle`/`DrawerDescription` dengan deskripsi `text-secondary-foreground` tanpa counter atau progress bar tambahan. Flow dibagi menjadi empat langkah: fakta utama, dampak & penanganan, konteks & risiko, lalu periksa. Progres ditunjukkan hanya melalui lingkaran indikator di footer: satu lingkaran active untuk langkah aktif dan lingkaran inactive untuk langkah lain, dengan label screen-reader `Langkah n dari 4`. Tombol Lanjut hanya aktif ketika field wajib pada langkah aktif valid; pengguna selalu dapat kembali tanpa kehilangan isian. Setiap pasangan label–field memakai wrapper `flex flex-col gap-2` (8px); untuk `fieldset`, legend dibuat block dengan `mb-2` agar jarak label ke komponen tetap tepat 8px dan tidak bergantung pada perilaku default browser. Risiko terkait dan detail pendukung tetap memakai `CollapsibleCard` sebagai progressive disclosure. Kejadian boleh disimpan sebagai `Belum dipetakan`. Langkah Periksa merangkum record sebelum `AlertDialog` menjelaskan bahwa penyimpanan tidak dapat dibatalkan. Setelah tersimpan, detail LED bersifat read-only; label metadata pada detail memakai 13px (`text-[13px]`) dengan `text-muted-foreground`, sedangkan nilai detail tetap 14px. Satu-satunya mutasi adalah menambahkan hubungan risiko melalui dialog dengan konfirmasi permanen. `AppHeader` menjadi satu-satunya pemilik judul/subtitle halaman; halaman detail mengirim aksi ke title row melalui `actionsPlacement="top"` tanpa merender header lokal kedua. Collection `/risk-events` mengikuti toolbar dan tabel Penanganan: `CollectionTableCard`, header netral 40px, row dua baris dengan padding compact, kolom terpisah berurutan Kode, Kejadian, Tingkat, Risiko terkait, Dicatat oleh, dan Waktu di trailing edge, kode monospace, serta warna yang dibatasi pada badge tingkat kejadian. Empty state tanpa data menjelaskan langkah berikutnya, sedangkan hasil pencarian menyebutkan kata kunci yang dicari dan menyediakan aksi `Hapus pencarian`; error pemuatan memakai pesan yang dapat dipahami pengguna dan aksi `Coba lagi` tanpa menampilkan detail teknis backend.

### Dropdowns and Popovers
- **Style:** Compact solid surfaces with an 8px radius, a 4px inner inset, small offset from trigger, and the Card `border-shadow` boundary. Keep option rows at 32px (`h-8`) with an 8px radius; use 8px left padding and reserve 40px on the right for selectable indicators. Use a quiet neutral fill for hover/focus and a trailing checkmark for selection. Avoid relying on translucency for content legibility. Keep the canonical trigger at 36px (`h-9`). The shared `PopoverSelectField` used by risk forms, risk filters, and assessment fields delegates to this same DropdownMenu panel/radio-item geometry.
...
- **Do** use rounded-lg for controls, cards, dialogs, and standard surfaces; use rounded-md for tab buttons and sidebar menu items.
