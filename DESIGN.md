...
- **Form section title/subtitle** (`text-sm font-medium tracking-tight` / `text-xs leading-relaxed`): Section card headings inside the risk form use a 14px medium title and a 12px muted supporting description so the form hierarchy stays calm and scannable.
...
- **Intelligence collection pages** such as MoM and Document Intelligence use the same `PageStack` + `CollectionToolbar` shell as the risk register. Keep their workflow cards on the neutral `rounded-xl` collection surface, with actions and review states inside that shared rhythm.
