...
- [ ] `curl PUT /api/v1/organizations/:id` with `context` field saves and retrieves correctly
- [ ] `curl PUT` with >2000 chars returns 400
- [ ] Any AI endpoint called after setting context includes that context in AI system message
- [ ] Empty context = no injection (existing behavior preserved)
