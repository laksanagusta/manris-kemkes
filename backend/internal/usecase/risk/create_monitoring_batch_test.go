...
func (r *fakeMonitoringRiskRepo) GetKRIBreachSummary(context.Context, []uuid.UUID) ([]entity.KRIBreachItem, error) {
	return nil, nil
}
...
func TestCreateMonitoringBatchUseCase_ExecuteCreatesDrafts(t *testing.T) {
	...
	uc := NewCreateMonitoringBatchUseCase(riskRepo, &fakeMonitoringUserRepo{})
...
func TestCreateMonitoringBatchUseCase_RejectsInvalidCycle(t *testing.T) {
	...
	uc := NewCreateMonitoringBatchUseCase(&fakeMonitoringRiskRepo{}, &fakeMonitoringUserRepo{})
...
func TestCreateMonitoringBatchUseCase_ComputesWeightAndNilai(t *testing.T) {
	...
	expectedScore := int(float64(2) * float64(3) * expectedWeight)
	if draft.InherentScore != expectedScore {
		t.Fatalf("expected InherentScore %d, got %d", expectedScore, draft.InherentScore)
	}
}
...
func TestIntegration_TemplatePreviewBatchCreate(t *testing.T) {
	...
	batchUC := NewCreateMonitoringBatchUseCase(riskRepo, userRepo)
