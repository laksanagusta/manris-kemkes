	...
	if input.Action == "approve" {
		...
		if nextStep == nil {
			...
			if approvalReq.RequestType == "risk" && newEntityStatus == entity.RiskStatusApproved && uc.taskRepo != nil {
				approvedRisk, err := uc.riskRepo.GetByID(ctx, approvalReq.EntityID, input.OrgIDs)
				if err != nil {
					return nil, domainerrors.Wrap(err, "failed to load approved risk")
				}
				if _, err := mtuc.NewEnsureTasksForRiskVersionUseCase(uc.taskRepo, uc.riskRepo).Execute(ctx, approvalReq.EntityID, approvedRisk.AssessmentCycle, input.OrgIDs); err != nil {
