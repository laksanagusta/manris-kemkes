package incident

import (
	"context"
	"testing"

	"github.com/google/uuid"
	"github.com/manris/backend/internal/domain/entity"
	"github.com/manris/backend/internal/domain/repository"
	"github.com/manris/backend/internal/domain/service"
)

type stubIncidentRepo struct {
	listOrgIDs []uuid.UUID
}

func (r *stubIncidentRepo) Create(context.Context, *entity.Incident) error { return nil }
func (r *stubIncidentRepo) GetByID(context.Context, string, []uuid.UUID) (*entity.Incident, error) {
	return nil, nil
}
func (r *stubIncidentRepo) Update(context.Context, *entity.Incident) error { return nil }
func (r *stubIncidentRepo) Delete(context.Context, string) error           { return nil }
func (r *stubIncidentRepo) List(_ context.Context, orgIDs []uuid.UUID) ([]*entity.Incident, error) {
	r.listOrgIDs = append([]uuid.UUID(nil), orgIDs...)
	return []*entity.Incident{}, nil
}
func (r *stubIncidentRepo) GetSummary(_ context.Context, orgIDs []uuid.UUID) (map[string]interface{}, error) {
	r.listOrgIDs = append([]uuid.UUID(nil), orgIDs...)
	return map[string]interface{}{}, nil
}

type stubOrgRepo struct{}

func (r *stubOrgRepo) Create(context.Context, *entity.Organization) error { return nil }
func (r *stubOrgRepo) GetByID(context.Context, uuid.UUID) (*entity.Organization, error) {
	return nil, nil
}
func (r *stubOrgRepo) Update(context.Context, *entity.Organization) error   { return nil }
func (r *stubOrgRepo) Delete(context.Context, uuid.UUID) error              { return nil }
func (r *stubOrgRepo) List(context.Context) ([]*entity.Organization, error) { return nil, nil }
func (r *stubOrgRepo) ListWithFilter(context.Context, repository.OrganizationListFilter) ([]*entity.Organization, int, error) {
	return nil, 0, nil
}
func (r *stubOrgRepo) GetDescendants(context.Context, uuid.UUID) ([]uuid.UUID, error) {
	return nil, nil
}
func (r *stubOrgRepo) GetContext(context.Context, uuid.UUID) (string, error) {
	return "", nil
}

func TestListIncidents_PassesOrgIDsDirectly(t *testing.T) {
	repo := &stubIncidentRepo{}
	orgSvc := service.NewOrganizationHierarchy(&stubOrgRepo{})
	uc := NewListIncidentsUseCase(repo, orgSvc)

	orgA := uuid.New()
	orgB := uuid.New()

	_, err := uc.Execute(context.Background(), ListIncidentsInput{
		OrgIDs: []uuid.UUID{orgA, orgB},
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if len(repo.listOrgIDs) != 2 {
		t.Fatalf("expected 2 orgIDs, got %d", len(repo.listOrgIDs))
	}
	if repo.listOrgIDs[0] != orgA || repo.listOrgIDs[1] != orgB {
		t.Fatalf("orgIDs mismatch: got %v, want [%s, %s]", repo.listOrgIDs, orgA, orgB)
	}
}

func TestListIncidents_NilOrgIDsPassedThrough(t *testing.T) {
	repo := &stubIncidentRepo{}
	orgSvc := service.NewOrganizationHierarchy(&stubOrgRepo{})
	uc := NewListIncidentsUseCase(repo, orgSvc)

	_, err := uc.Execute(context.Background(), ListIncidentsInput{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if len(repo.listOrgIDs) != 0 {
		t.Fatalf("expected empty orgIDs, got %d", len(repo.listOrgIDs))
	}
}

func TestGetIncidentSummary_PassesOrgIDsDirectly(t *testing.T) {
	repo := &stubIncidentRepo{}
	uc := NewGetIncidentSummaryUseCase(repo)

	orgA := uuid.New()

	_, err := uc.Execute(context.Background(), GetIncidentSummaryInput{
		OrgIDs: []uuid.UUID{orgA},
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if len(repo.listOrgIDs) != 1 || repo.listOrgIDs[0] != orgA {
		t.Fatalf("orgIDs mismatch: got %v, want [%s]", repo.listOrgIDs, orgA)
	}
}
