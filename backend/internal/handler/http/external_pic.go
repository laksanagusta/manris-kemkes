...
func (h *ExternalPICHandler) Create(c *fiber.Ctx) error {
	...
	if input.Name == "" {
		return sendProblemDetails(c, 400, "Bad Request", "https://api.manris.com/errors/bad-request", "name is required")
	}
...
func (h *ExternalPICHandler) Delete(c *fiber.Ctx) error {
	...
	if err != nil {
		return sendProblemDetails(c, 400, "Bad Request", "https://api.manris.com/errors/bad-request", "invalid PIC ID")
	}
	...
	return c.JSON(fiber.Map{"message": "External PIC deleted successfully"})
}
