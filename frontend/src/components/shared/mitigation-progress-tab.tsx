...
export function MitigationProgressTab({ riskId, token }: MitigationProgressTabProps) {
  ...
  const fetchTasks = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.get<MitigationTask[]>(`/risks/${riskId}/tasks`, token);
      setTasks(data || []);
    } catch {
      // silent fail on 404
      setTasks([]);
  ...
  const handleGenerateTasks = async () => {
    try {
      const result = await api.post<{ tasksGenerated: number; tasksOverdue: number }>(
        "/mitigation-tasks/generate",
        {},
        token
      );
      toast.success(`${result.tasksGenerated} task baru dibuat, ${result.tasksOverdue} ditandai overdue`);
      fetchTasks();
    } catch {
  ...
  const groupedTasks = tasks.reduce<Record<string, MitigationTask[]>>((acc, task) => {
    ...
    return (
      <Card className="border-border/50">
        <CardContent className="flex items-center justify-center py-12">
          ...
          <span className="ml-2 text-sm text-muted-foreground">Memuat task mitigasi...</span>
        </CardContent>
      </Card>
    );
  }
      ...
      {tasks.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="rounded-xl border bg-card p-3 text-center">
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Total Task</p>
          </div>
          <div className="rounded-xl border bg-emerald-500/5 border-emerald-500/20 p-3 text-center">
            ...
            <p className="text-[10px] text-emerald-600 uppercase tracking-wider">Selesai</p>
          </div>
          <div className="rounded-xl border bg-amber-500/5 border-amber-500/20 p-3 text-center">
            <p className="text-2xl font-bold text-amber-600">{stats.pending}</p>
            <p className="text-[10px] text-amber-600 uppercase tracking-wider">Menunggu</p>
          </div>
          <div className="rounded-xl border bg-red-500/5 border-red-500/20 p-3 text-center">
            <p className="text-2xl font-bold text-red-600">{stats.overdue}</p>
            <p className="text-[10px] text-red-600 uppercase tracking-wider">Terlambat</p>
          </div>
        </div>
      )}
      ...
      <Card className="border-border/50">
        <CardHeader className="pb-3 flex flex-row items-center justify-between border-b border-border/50">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" className="gap-2 text-xs h-8" onClick={handleGenerateTasks}>
          ...
          {tasks.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-8 text-center border-2 border-dashed border-border/60 rounded-lg bg-muted/10">
              ...
              <p className="text-xs text-muted-foreground mt-1 max-w-sm">
                Klik &quot;Generate Task&quot; untuk membuat task berdasarkan jadwal mitigasi rutin, atau task akan dibuat otomatis oleh sistem setiap jam.
              </p>
              <Button variant="outline" size="sm" className="mt-4 gap-2 text-xs" onClick={handleGenerateTasks}>
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-xs font-semibold">{task.periodLabel}</span>
                              <Badge variant="outline" className={`text-[10px] gap-1 ${statusCfg.color}`}>
                                {statusCfg.icon} {statusCfg.label}
                              </Badge>
                            </div>
                            <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                              <span>Tenggat: {new Date(task.dueDate).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}</span>
                              {task.reportedByName && <span>Oleh: {task.reportedByName}</span>}
                            </div>
                            {task.status === "done" && (
                              <div className="mt-2">
                                <Progress value={task.progressPct} className="h-1.5" />
                                <div className="flex justify-between mt-1">
                                  <span className="text-[10px] text-muted-foreground">{task.progressPct}%</span>
                                  {task.notes && <span className="text-[10px] text-muted-foreground truncate max-w-[200px]">{task.notes}</span>}
                                </div>
                              </div>
                            )}
                          </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label className="text-xs">Persentase Penyelesaian <span className="text-destructive">*</span></Label>
              <div className="flex items-center gap-3">
                <Input
                  ...
                  aria-describedby={showValidationErrors && formErrors.progressPct ? "mitigation-progress-error" : undefined}
                />
                <span className="text-xs text-muted-foreground">%</span>
                <Progress value={Number(progressPct || 0)} className="flex-1 h-2" />
              </div>
              {showValidationErrors && formErrors.progressPct && (
                <p id="mitigation-progress-error" className="text-[11px] text-destructive">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                aria-describedby={showValidationErrors && formErrors.actualCost ? "mitigation-cost-error" : undefined}
              />
              {showValidationErrors && formErrors.actualCost && (
                <p id="mitigation-cost-error" className="text-[11px] text-destructive">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                aria-describedby={showValidationErrors && formErrors.evidenceUrl ? "mitigation-evidence-error" : undefined}
              />
              {showValidationErrors && formErrors.evidenceUrl && (
                <p id="mitigation-evidence-error" className="text-[11px] text-destructive">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                aria-describedby={showValidationErrors && formErrors.notes ? "mitigation-notes-error" : undefined}
              />
              {showValidationErrors && formErrors.notes && (
                <p id="mitigation-notes-error" className="text-[11px] text-destructive">
                  {formErrors.notes}
                </p>
              )}
            </div>
