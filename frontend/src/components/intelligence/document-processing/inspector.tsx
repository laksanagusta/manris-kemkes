...
function documentStatus(document: ProcessingJob["documents"][number]) {
  switch (document.status) {
    case "indexed":
      return { label: "Terindeks", tone: "success" as const };
    case "failed":
      return { label: "Gagal", tone: "danger" as const };
    case "warning":
      return { label: "Perlu perhatian", tone: "warning" as const };
    case "indexing":
      return { label: "Mengindeks", tone: "progress" as const };
    default:
      return { label: "Siap", tone: "neutral" as const };
  }
}

function severityMeta(severity: Finding["severity"]) {
  if (severity === "critical" || severity === "high") return { label: severity === "critical" ? "Kritis" : "Tinggi", tone: "danger" as const };
  if (severity === "medium") return { label: "Sedang", tone: "warning" as const };
  return { label: "Rendah", tone: "neutral" as const };
}
...
function Meta({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0"><div className="font-display text-xs uppercase tracking-[0.1em] text-muted-foreground">{label}</div><div className="mt-1 truncate text-xs font-medium text-foreground" title={value}>{value}</div></div>;
}
