...
function toDelimited(value?: string[]) {
	return Array.isArray(value) ? value.filter(Boolean).join(EXCEL_LINE_BREAK) : "";
}
...
function createBaseRow(risk: RiskExportItem): BulkRiskExportRow {
	return {
		...
		"Efektivitas Pengendalian": controlEffectivenessLabel(risk.controlEffectiveness),
		P: risk.probability ?? "",
		D: risk.impact ?? "",
		Bobot: risk.weight ?? "",
...
export function buildRiskBulkExportRows(risks: RiskExportItem[]): BulkRiskExportRow[] {
	return risks.map((risk) => {
		const baseRow = createBaseRow(risk);

		return {
			...baseRow,
			"RPR Uraian": joinMitigationValues(risk.mitigations, (mitigation) => mitigation.action || ""),
			"PIC RPR": joinMitigationValues(risk.mitigations, (mitigation) => mitigation.owner || ""),
			"Jadwal Pelaksanaan": joinMitigationValues(risk.mitigations, (mitigation) => mitigationSchedule(mitigation)),
		};
	});
}

export async function createRiskBulkExportWorkbookBuffer(risks: RiskExportItem[], cycle: string) {
	...
	sheet.autoFilter = {
		from: "A1",
		to: String.fromCharCode(64 + BULK_RISK_EXPORT_COLUMNS.length) + Math.max(rows.length + 1, 1),
	};
...
export function exportRiskBulkCSV(risks: RiskExportItem[], cycle: string) {
	const rows = buildRiskBulkExportRows(risks);
	const worksheet = XLSX.utils.json_to_sheet(rows, { header: [...BULK_RISK_EXPORT_COLUMNS] });
	const csv = XLSX.utils.sheet_to_csv(worksheet);
	downloadBlob(new Blob([csv], { type: "text/csv;charset=utf-8;" }), `risk-register-${cycle}.csv`);
}

export async function exportRiskBulkXLSX(risks: RiskExportItem[], cycle: string) {
