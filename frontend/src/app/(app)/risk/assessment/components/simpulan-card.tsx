...
export function SimpulanCard({ nilaiCurrent, nilaiBaru, targetScore = 0, probability = 1, impact = 1 }: SimpulanCardProps) {
  ...
  const deltaPercent = nilaiCurrent > 0 ? Math.round((delta / nilaiCurrent) * 100) : 0;
  
  const TrendIcon = isStable ? Minus : (isDecrease ? TrendingDown : TrendingUp);
  const trendColorClass = isStable ? "text-muted-foreground" : (isDecrease ? "text-success" : "text-risk-extreme");

  const progress = targetScore > 0 && nilaiCurrent !== targetScore
    ? Math.min(100, Math.max(0, Math.round(((nilaiCurrent - nilaiBaru) / (nilaiCurrent - targetScore)) * 100)))
    : 0;
  ...
  return (
    <div className="space-y-4">
      <div className={cn("flex items-center justify-between rounded-lg border p-4", levelColorClass)}>
        ...
        <div className="text-right">
          ...
          <p className="text-xs font-mono opacity-80">Skor Risiko: {Math.round(nilaiBaru)}</p>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="pt-6 space-y-4">
          <div>
            <p className="text-xs font-medium text-muted-foreground mb-1">Tingkat Risiko</p>
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-foreground">
                {Math.round(nilaiCurrent)} &rarr; {Math.round(nilaiBaru)}
              </span>
              <span className={cn("text-xs flex items-center font-medium", trendColorClass)}>
                <TrendIcon className="size-3.5 mr-1" />
                ({delta > 0 ? "+" : ""}{Math.round(delta)}, {deltaPercent > 0 ? "+" : ""}{deltaPercent}%)
              </span>
            </div>
          </div>
          ...
          {targetScore > 0 && (
            <div className="border-t border-border/50 pt-4">
              <div className="flex justify-between items-end mb-2">
                <p className="text-xs font-medium text-muted-foreground">vs Target Penurunan</p>
                <div className="text-right">
                  <span className="text-xs font-medium text-foreground">
                    {Math.round(nilaiBaru)} / {Math.round(targetScore)}
                  </span>
                  <span className="text-xs text-muted-foreground ml-1">
                    ({Math.max(0, Math.round(nilaiBaru - targetScore))} gap)
                  </span>
                </div>
              </div>
